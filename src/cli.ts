...
import type { UploadAll, UploadImport } from './types'
...
prog
  ...
  .action(async (opts: { from?: string }) => {
    try {
      const kp = await ((opts.from ?? '').length > 0
...
prog
  ...
  .action(
    async (opts: {
      ...
      try {
        const kp = await ((opts.issuer ?? '').length > 0
        ...
        const thisCan: UploadAll['can'] | UploadImport['can'] = (opts.with ??
          cap.with) as any
